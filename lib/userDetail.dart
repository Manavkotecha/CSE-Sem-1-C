...
import 'package:matrimony3/addUser.dart';
...
class _UserDetailState extends State<UserDetail> {
      ...
      MaterialPageRoute(
        builder: (context) => AddUser(
          onSaveUser: (updatedUserData) {
            setState(() {
              userEdit = Map<String, dynamic>.from(userEdit); // ✅ Ensure it's mutable
              userEdit.addAll(updatedUserData);
              userEdit['age'] = calculateAge(userEdit['dob']);
              _user = userEdit; // ✅ Update _user to reflect changes
            });
  ...
  String formatHobbies(dynamic hobbies) {
    ...
    if (hobbies is String) {
      return hobbies; // If already a string, return as is
    } else if (hobbies is List<String>) {
      return hobbies.join(", "); // Convert list to a comma-separated string
    }
    return "N/A";
  }
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildDetail(Icons.favorite, "Hobbies", formatHobbies(_user['hobbies'])),
                  ],
                ),
              ),
              SizedBox(height: 20),
